...
const balance = (msg = Discord.Message, reply, floor, currency = Discord.Collection) => {
  const target = msg.mentions.users.first() || msg.author;
  return reply(msg.channel.id, `${target} has ${floor(currency.getBalance(target.id))}🍰(${currency.getBalance(target.id)}🍰)`, '#ffffba');
};
